...
n64Button buttons[buttonsLength] =
{
    ...
    // {TRIGGER_Z_PIN,     BUTTON_9},  // TRIGGER_Z_PIN não declarado
...
void app_loop(void *params)
{
    while (true)
    {
        if (bleGamepad.isConnected())
        {
            ...
            // Analógico C-stick (comentado - constantes não declaradas)
            // btn_x_axis = btn_y_axis = 0;
            ...
            // Joystick (comentado - funções e constantes não declaradas)
...
extern "C" void app_main(void)
{
    ...
    // bleGamepadConfig.setAxesMax(JOYSTICK_ABS_MAX * 2);  // JOYSTICK_ABS_MAX não declarado
    bleGamepadConfig.setAxesMax(0xFFFF);  // Valor padrão
